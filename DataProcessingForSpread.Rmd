...
AllGames <- tmpDF
write.table(AllGames, "/home/kegiles/ToyData/AllGames.tsv", sep = "\t", col.names = T, row.names = F, quote = F)
AllGames <- read.table("./AllGames.tsv", sep = "\t", header = T)
```
...
DifferentialDF <- subset(DifferentialDF, select = -c(Home,Away,HomeScore,AwayScore))
colnames(DifferentialDF)
```
...
library(zoo)
DifferentialDF_MA5[3:38] <- lapply(DifferentialDF[3:38], function(x) rollmean(x = x, k = 5, align = "right", fill = NA))
#add the actual score to regress against alongside the rolling average of score differential 
DifferentialDF_MA5 <- cbind(DifferentialDF[c(38)],DifferentialDF_MA5)
colnames(DifferentialDF_MA5) <- c("NetScore","UniqueGameID","date",paste0(colnames(DifferentialDF_MA5)[4:39],"_MA5"))
#colnames(DifferentialDF_MA5)
write.table(DifferentialDF_MA5, file = "/home/kegiles/ToyData/DifferentialStatsForSpread.tsv", col.names = T, row.names = F, quote = F, sep = "\t")
```
