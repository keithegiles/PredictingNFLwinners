...
library(jsonlite)
NFLgamesJSON <- fromJSON("games_1512362753.8735218.json")
...
DifferentialDF <- subset(DifferentialDF, select = -c(UniqueGameID,date,Home,Away,HomeScore,AwayScore))
#The last step is to calculate the 5 game rolling average for each team 
tmp <- DifferentialDF
library(zoo)
tmp[1:35] <- lapply(DifferentialDF[1:35], function(x) rollmean(x = x, k = 5, align = "right", fill = NA))
DifferentialDF_MA5 <- na.omit(tmp)
DifferentialDF_MA5[1:5,1:5]
head(DifferentialDF_MA5)
```
